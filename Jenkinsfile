pipeline {
  ...
  environment {
    ...
    BUILD_VERSION_ARG = 'ENDLESSH_RELEASE'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-endlessh'
    DOCKERHUB_IMAGE = 'linuxserver/endlessh'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/endlessh'
    PR_DOCKERHUB_IMAGE = 'lspipepr/endlessh'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='true'
    CI_WEB='false'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/endlessh") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/endlessh",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-endlessh",\
                       "backendUrl":"https://ghcr.io/linuxserver/endlessh",\
                       "publicUrl":"https://lscr.io/linuxserver/endlessh"}' || :
              else
